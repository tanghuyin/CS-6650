  ...
  public static void main(String[] args) throws InterruptedException {
    ...
    for (int i = 0; i < phase1Threads; i++) {
      new Thread(new LiftRideThread(skiersPerThread * i + 1,
          Math.min(skiersPerThread * (i + 1), numOfSkiers), startTime, endTime, numOfSkiLifts, requestCount, phase1CountDown, allCountDown1, counter1)).start();
    }
    ...
    for (int i = 0; i < phase2Threads; i++) {
      new Thread(new LiftRideThread(skiersPerThread * i + 1,
          Math.min(skiersPerThread * (i + 1), numOfSkiers), startTime, endTime, numOfSkiLifts, requestCount, phase2CountDown, allCountDown2, counter2)).start();
    }
    ...
    for (int i = 0; i < phase3Threads; i++) {
      new Thread(new LiftRideThread(skiersPerThread * i + 1,
          Math.min(skiersPerThread * (i + 1), numOfSkiers), startTime, endTime, numOfSkiLifts, requestCount, phase3CountDown, allCountDown3, counter3)).start();
    }
    ...
    logger.info("Number of unsuccessful requests: {}", failureCounter.get());
